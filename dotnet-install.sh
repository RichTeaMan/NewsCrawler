...
construct_download_link() {
    ...
    temporary_file_template="${temporary_file_template}.zip"

    local download_link=null
    if [[ "$runtime" == "dotnet" ]]; then
        download_link="$azure_feed/Runtime/$specific_version/dotnet-runtime-$specific_version-$osname-$normalized_architecture.$extension"
    elif [[ "$runtime" == "aspnetcore" ]]; then
        download_link="$azure_feed/aspnetcore/Runtime/$specific_version/aspnetcore-runtime-$specific_version-$osname-$normalized_architecture.$extension"
    elif [ -z "$runtime" ]; then
        download_link="$azure_feed/Sdk/$specific_version/dotnet-sdk-$specific_version-$osname-$normalized_architecture.$extension"
    else
        return 1
    fi
...
extract_dotnet_package() {
    ...
    then
        ...
        # unzip seems to return 1. I'm choosing to ignore this.
        unzip "$zip_path" -d "$temp_out_path" > /dev/null || true
    fi
